...
OUTPUT_DIR = r"c:/Users/Nandhini Prbakaran/OneDrive/Documents/Minor_Nan_Audio/Minor_Nan_Audio/results_proposed"
BATCH_SIZE = 32
EPOCHS = 30
LEARNING_RATE = 0.001
...
class SERDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        feature = feature[np.newaxis, ...] # (1, 128, 188)
...
def evaluate(model, loader, criterion):
    ...
    ua = recall_score(all_targets, all_preds, average='macro')
...
def run_experiment(ablation='none'):
    print(f"Running Experiment: CNN-Transformer (Ablation: {ablation})")
    ...
    exp_dir = os.path.join(OUTPUT_DIR, ablation)
    ensure_dir(exp_dir)
    ...
    for train_idx, val_idx in gkf.split(indices, groups=groups):
        ...
        model = CNNTransformer(num_classes=6, ablation=ablation).to(DEVICE)
        
        criterion = nn.CrossEntropyLoss()
        optimizer = optim.Adam(model.parameters(), lr=LEARNING_RATE)
        
        best_val_ua = 0.0
        best_model_path = os.path.join(exp_dir, f"fold{fold}_best.pth")
        ...
        plot_confusion_matrix(v_cm, f"Val CM Fold {fold}", os.path.join(exp_dir, f"fold{fold}_val_cm.png"))
        
        _, t_wa, t_ua, t_f1, t_cm, _, _ = evaluate(model, tess_loader, criterion)
        plot_confusion_matrix(t_cm, f"TESS CM Fold {fold}", os.path.join(exp_dir, f"fold{fold}_tess_cm.png"))
    ...
    res_df.to_csv(os.path.join(exp_dir, "metrics.csv"), index=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--ablation', type=str, default='none', choices=['none', 'no_transformer', 'no_attention'])
    args = parser.parse_args()
    
    run_experiment(args.ablation)
